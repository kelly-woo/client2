(function() {
  ...
  function run(configuration, AnalyticsHelper) {  
    //analytics start
    _initMixPanel();
  ...
  function _initMixPanel() {
      ...
      mixpanel.init(configuration.mp_member_token, {'loaded': function() {
        if (mixpanel) {
          if (mixpanel.cookie && _.isFunction(mixpanel.cookie.clear)) {
            mixpanel.cookie.clear();
          }
          if (_.isFunction(mixpanel.identify)) {
            mixpanel.identify(UUID());
          }
        }
      }});
      mixpanel.init(configuration.mp_account_token, {'loaded': function() {
        if (mixpanel && mixpanel.account) {
          if (mixpanel.account.cookie && _.isFunction(mixpanel.account.cookie.clear)) {
            mixpanel.account.cookie.clear();
          }
          if (_.isFunction(mixpanel.account.identify)) {
